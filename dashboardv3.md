...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ::-webkit-scrollbar-thumb { background: #2a2000; }
...
function BootLine({ line }) {
  if (line.ok) return (
    <div className="boot-line" style={{ display: 'flex', justifyContent: 'space-between', color: '#5a4a20', fontSize: '13px', lineHeight: '1.7' }}>
      <span>{line.text}</span>
      <span style={{ color: '#e8a320', marginLeft: '24px' }}>[ OK ]</span>
    </div>
  );
  return (
    <div className="boot-line" style={{ color: line.color || '#3a3020', fontSize: '13px', lineHeight: '1.7' }}>
      {line.text || '\u00a0'}
    </div>
  );
}
...
function Prompt({ children, showCursor = false }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', lineHeight: '1.7' }}>
      ...
      <span style={{ color: '#3a3020', userSelect: 'none' }}>@</span>
      <span style={{ color: '#7ab0c0', userSelect: 'none' }}>netwatch</span>
      <span style={{ color: '#3a3020', userSelect: 'none' }}>~</span>
      <span style={{ color: '#5a4a20', userSelect: 'none' }}>$</span>
...
function DashOutput({ devices, alerts, selectedDev, selectedMac, onSelect, activeOnly, setActiveOnly, onUpdate, onResolve }) {
  ...
  return (
    <div className="dash-enter" style={{ fontSize: '13px', lineHeight: 1.75 }}>
      ...
      {/* summary */}
      <div style={{ color: '#5a4a20', marginBottom: '4px' }}>
        {`  >> netmonitor activated — ${devices.length} devices found, ${activeCount} active, ${alerts.length} alert(s)`}
      </div>
      <div style={{ color: '#2a2000', marginBottom: '16px' }}>
        {'  ─'.repeat(32)}
      </div>
      ...
      {asciiTable(devices).map((line, i) => {
        const isDataRow = i > 2 && i < asciiTable(devices).length - 1;
        const rowIndex  = i - 3;
      ...
      <div style={{ color: '#2a2000', margin: '16px 0 4px' }}>{'  ─'.repeat(32)}</div>

      {/* alerts */}
      <div style={{ color: '#7ab0c0', marginBottom: '4px' }}>{'  // ALERT LOG'}</div>
      {asciiAlerts(alerts).map((line, i) => (
        <div key={i} style={{ color: alerts[i] ? '#ff4d4f' : '#3a3020' }}>{line}</div>
      ))}

      <div style={{ color: '#2a2000', margin: '16px 0 4px' }}>{'  ─'.repeat(32)}</div>

      {/* rssi */}
      <div style={{ color: '#7ab0c0', marginBottom: '4px' }}>{'  // SIGNAL STRENGTH'}</div>
      {asciiSparkline(selectedDev).map((line, i) => (
        <div key={i} style={{ color: '#c8a870' }}>{line}</div>
      ))}

      <div style={{ color: '#2a2000', margin: '16px 0 4px' }}>{'  ─'.repeat(32)}</div>
      <div style={{ color: '#3a3020' }}>
...
export default function Dashboard() {
  ...
  const [history,      setHistory]      = useState([]);  // [{prompt, output}]
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            zIndex: 50,
            background: '#0f0f0f',
            borderBottom: collapsed ? '1px solid #2a2000' : 'none',
          ...
          {collapsed ? (
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <span style={{ color: '#2a2000' }}>]─[</span>
                <span style={{ color: '#5a4a20', fontSize: '11px' }}>root@netwatch</span>
                <span style={{ color: '#2a2000' }}>]</span>
                <span style={{ color: '#3a3020', fontSize: '11px' }}>// active</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px', fontSize: '11px' }}>
                <span style={{ color: '#3a3020' }}>DEV[{String(devices.length).padStart(2,'0')}]</span>
                <span style={{ color: alerts.length > 0 ? '#ff4d4f' : '#3a3020' }}>
                ...
                <button
                  ...
                  style={{
                    background: 'transparent', border: '1px solid #2a2000',
                    color: '#5a4a20', fontFamily: "'IBM Plex Mono'", fontSize: '10px',
            ...
            /* full terminal */
            <div style={{ padding: '24px 28px', minHeight: activated ? 'auto' : '100vh', display: 'flex', flexDirection: 'column' }}>
              ...
              {history.map((entry, i) => (
                <div key={i}>
                  ...
                  {entry.output && entry.output !== '__CLEAR__' && (
                    <div style={{ color: '#5a4a20', fontSize: '13px', lineHeight: 1.7, paddingLeft: '4px' }}>
                      {entry.output}
                    </div>
                  )}
                </div>
            ...
            <div className="dash-enter">
              ...
              <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
                {[
                  ...
                  { label: 'OPEN ALERTS',    val: alerts.length,  accent: alerts.length > 0 ? '#ff4d4f' : '#3a3020' },
                ].map(s => (
                  <div key={s.label} style={{
                    ...
                    <div style={{ fontSize: '10px', color: '#3a3020', letterSpacing: '0.2em', marginBottom: '6px' }}>{s.label}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '20px', alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ background: '#0a0a0a', border: '1px solid #1a1500' }}>
                    <div style={{ padding: '14px 20px', borderBottom: '1px solid #1a1500', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: '10px', color: '#3a3020', letterSpacing: '0.2em', marginBottom: '4px' }}>├── DEVICE_INVENTORY</div>
                        <div style={{ fontSize: '15px', color: '#c8a870' }}>
                          <span style={{ color: '#5a4a20' }}>$</span> ls -la /dev/endpoints
                        </div>
                      </div>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#5a4a20', fontSize: '11px', cursor: 'pointer', border: '1px solid #1a1500', padding: '6px 12px' }}>
                  ...
                  <div style={{ background: '#0a0a0a', border: '1px solid #1a1500', padding: '18px' }}>
                    <div style={{ fontSize: '10px', color: '#3a3020', letterSpacing: '0.2em', marginBottom: '4px' }}>└── RSSI_HISTORY</div>
                    <div style={{ fontSize: '13px', color: '#5a4a20', marginBottom: '14px' }}>
                      <span style={{ color: '#5a4a20' }}>$</span> plot --signal{selectedDev ? ` --mac=${selectedDev.mac}` : ' --select-device'}
                    </div>
                ...
                <div style={{ background: '#0a0a0a', border: '1px solid #1a1500' }}>
                  <div style={{ padding: '14px 20px', borderBottom: '1px solid #1a1500', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: '10px', color: alerts.length > 0 ? '#ff4d4f44' : '#3a3020', letterSpacing: '0.2em', marginBottom: '4px' }}>├── ALERT_DAEMON</div>
                      <div style={{ fontSize: '15px', color: '#c8a870' }}>
                        <span style={{ color: '#5a4a20' }}>$</span> tail -f /var/log/anomaly
                      </div>
                    </div>
                    {alerts.length > 0 && (
                  ...
                  <div style={{ padding: '12px' }}>
                    {alerts.length === 0 && (
                      <div style={{ padding: '20px', textAlign: 'center', fontSize: '11px', color: '#2a2000', letterSpacing: '0.15em' }}>
