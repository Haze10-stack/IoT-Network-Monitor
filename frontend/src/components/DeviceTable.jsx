...
// Colour-code vendor: known brand = blue, Unknown = muted
function VendorBadge({ vendor }) {
  ...
  return (
    <span style={{
      fontFamily:      "'DM Sans'",
      fontSize:        '11px',
      color:           isUnknown ? '#484f58' : '#58a6ff',
      background:      isUnknown ? '#0d1117' : '#0d1f36',
      border:          `1px solid ${isUnknown ? '#21262d' : '#1f4068'}`,
      borderRadius:    '4px',
      padding:         '2px 7px',
      whiteSpace:      'nowrap',
    }}>
...
export default function DeviceTable({ devices, onSelect, selectedMac, onUpdate }) {
  ...
  const TH = ({ children }) => (
    <th style={{
      ...
      fontFamily: "'DM Sans'", fontSize: '11px', color: '#484f58',
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {devices.map(d => {
            ...
            const latestRssi = d.rssiHistory?.at(-1)?.rssi ?? null;
            ...
            return (
              ...
              >
                ...
                {/* RSSI */}
                <td style={{ padding: '10px 12px' }}>
                  {latestRssi !== null ? rssiBar(latestRssi) : '—'}
                </td>
